...
namespace Orchid.Services
{
    public class OrchidWebAPIProxy
    {
        ...
        */
        ...
        //This method call the UpdateCharacter web API on the server and return a boolean if the change was a seccess
        //or null if the call fails

        public async Task<bool> UpdateCharacter(Character cr)
        {
            //Set URI to the specific function API
            ...
            try
            {
                ...
                string json = JsonSerializer.Serialize<Character>(cr, options);
                StringContent content = new StringContent(json, Encoding.UTF8, "application/json");
                HttpResponseMessage response = await this.client.PostAsync(url, content);
        ...
        //This method call the DeleteCharacter web API on the server and return a boolean if the change was a seccess
        //or null if the call fails
        public async Task<bool> DeleteCharacter(Character u)
        {
            //Set URI to the specific function API
            string url = $"{this.baseUrl}deleteCharacter";
            try
            {
                JsonSerializerOptions options = new JsonSerializerOptions
                {
                ...
                string json = JsonSerializer.Serialize<Character>(u, options);
                StringContent content = new StringContent(json, Encoding.UTF8, "application/json");
                HttpResponseMessage response = await this.client.PostAsync(url, content);
        ...
        //This method call the GetAllCharacters web API on the server and return a list of every Character in the data base
        //or null if the call fails
        ...
        //This method call the SendPrompt web API on the server and returns an imgId to an image generated by chatGPT using the prompt
        //or null if the call fails
        public async Task<string> SendPrompt(Character cr)
        {
            //Set URI to the specific function API
            string url = $"{this.baseUrl}sendPrompt";
            try
            {
                //Call the server API
                ...
                if (response.IsSuccessStatusCode)
                {
                    cr.ImgId = OrchidWebAPIProxy.ImageBaseAddress += $"/CharacterImages/Iu{cr.UserId}c{cr.Id}";
                    //Extract the content as string
                    string resContent = await response.Content.ReadAsStringAsync();
                    //Desrialize result
                    JsonSerializerOptions options = new JsonSerializerOptions
                ...
                {
                    return "Chat GPT didnt like your prompt, please try again!";
                }
            }
            catch (Exception ex)
            {
